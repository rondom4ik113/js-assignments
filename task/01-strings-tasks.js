...
function getRectangleString(width, height) {
    ...
    let betweenPart = '│' + ' '.repeat(width - 2) + '│\n';
    let rectangle = topSide + betweenPart.repeat(height - 2) + bottomSide;
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++) {        
        if (ABC.indexOf(str[i].toUpperCase()) === -1) {
            Rts += str[i];
            continue;
        }

        let isTopCase = (str.charCodeAt(i) >= 65 && str.charCodeAt(i) <= 90) ? true : false;

        let index = ABC.indexOf(str[i].toUpperCase());
        let newIndex = (index + 13) % ABC.length;
        let result = ABC[newIndex];        

        Rts += isTopCase ? result : result.toLowerCase();        
    }    
